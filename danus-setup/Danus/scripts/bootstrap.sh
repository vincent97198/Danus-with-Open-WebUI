...
if [ -n "$CODEX_JS" ] && [ "$("$NODE_DIR/bin/node" "$CODEX_JS" --version 2>/dev/null)" = "codex-cli $CODEX_VERSION" ]; then
  log "codex present: $CODEX_JS"
else
  log "installing @openai/codex@$CODEX_VERSION -> $CODEX_NPM"
  mkdir -p "$CODEX_NPM"
  $NICE "$NODE_DIR/bin/npm" install -g --prefix "$CODEX_NPM" "@openai/codex@$CODEX_VERSION" >/dev/null 2>&1 \
